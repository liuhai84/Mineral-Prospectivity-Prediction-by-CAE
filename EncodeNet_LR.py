...
'''测试''' 
def test_LR(test_data, test_label,model):
    ...
    np.savetxt(r"结果\LR验证.txt", result, delimiter = "\t")
    

'''预测'''    
def predict_LR(model):
    p_LR=model.predict(predict_data)
    pre_p_LR=model.predict_proba(predict_data)[:,1]
    result = np.array([p_LR, pre_p_LR]).T
    np.savetxt(r"结果\LR预测.txt", result, delimiter = "\t")
...
#    train_curve(LR)
    ...
    #测试准确率
    print('20%varify准确率：')
    test_LR(varify_data, varify_label,LR)
    
    print('test准确率：')
    test_LR(test_data, test_label,LR)
